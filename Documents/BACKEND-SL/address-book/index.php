...
<body>
            ...
            <table class="highlight centered">
                ...
                <tbody>
                    <?php foreach ($rows as $row): ?>
                        <tr>
                            <td><img class="circle" 
                                    src="images/users/<?=getImageName($row['image_name'], $row['id'] ); ?>" 
                            ...
                            <td><?= $row['telephone']; ?></td>
                            <td><?= $row['address']; ?></td>
                            <td><a class="btn btn-floating green lighten-2" href="edit-contact.php?id=<?= $row['id']; ?>">
                                <i class="material-icons">edit</i>
                                </a></td>
                            <td><a class="btn btn-floating red lighten-2 modal-trigger deleteBtn"
                                    data-id=<?= $row['id'] ?> href="#deleteModal"><i
        ...
        <div class="col s12">
            <ul class="pagination">
                ...
                <?php else: ?>
                    <li>
                        <a
                            href="http://localhost:9999/index.php?<?= construct_query_params(array_merge($_GET, ['page' => $page - 1])); ?>"><i
                ...
                <li class="<?= ($i === $page) ? 'active' : '' ?>"><a
                        href="http://localhost:9999/index.php?<?= construct_query_params(array_merge($_GET, ['page' => 1])); ?>"><?= 1 ?></a>
                </li>
                ...
                $end = min($total_num_of_pages - 1, $page + 2);
                ?>

                <?php for ($i = $start; $i <= $end; $i++): ?>
                    <li class="<?= ($i === $page) ? 'active' : '' ?>"><a
                            href="http://localhost:9999/index.php?<?= construct_query_params(array_merge($_GET, ['page' => $i])); ?>"><?= $i ?></a>
                    </li>
                <?php endfor; ?>


                <?php if (!($page >= $total_num_of_pages - 3) || !$page == $total_num_of_pages): ?>
                    <li class="disabled">
                        <a href="#"><i class="material-icons">...</i></a>
                    </li>
                <?php endif; ?>
                ...
                <li class="<?= ($i === $page) ? 'active' : '' ?>"><a
                        href="http://localhost:9999/index.php?<?= construct_query_params(array_merge($_GET, ['page' => $total_num_of_pages])); ?>"><?= $total_num_of_pages ?></a>
                </li>
    ...
    <div id="deleteModal" class="modal">
        ...
        <div class="modal-footer">
            ...
            <form method="post" action="delete-contact.php" style="display : inline-block">
